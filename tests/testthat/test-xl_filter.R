...
test_that("there is no coercion between text and numbers", {
  # Excel: a numeric criteria against a text column matches nothing at all
  df <- data.frame(t = c("10", "20", "abc"), stringsAsFactors = FALSE)
  expect_equal(kept(df, xl_filter("t", ">", 15)), integer(0))
  expect_equal(kept(df, xl_filter("t", "==", 10)), integer(0))
  # the mirror case is inferred by symmetry rather than measured
  num <- data.frame(q = c(10, 20))
  expect_equal(kept(num, xl_filter("q", "==", "10")), integer(0))
})
...
test_that("a filter on a formula column is refused", {
