...
#' @param freeze_panes Should headers be frozen?
...
write_xlsx <- function(x, path = tempfile(fileext = ".xlsx"), col_names = TRUE,
                       format_headers = TRUE, use_zip64 = FALSE, freeze_panes = format_headers){
  if(is.data.frame(x))
    x <- list(x)
  ...
  ret <- .Call(C_write_data_frame_list, x, path, col_names, format_headers, use_zip64, freeze_panes)
  invisible(ret)
}
